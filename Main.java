...
public class Main{
    ...
    public static void showFarmInformation(){
        ...
        lineBreaker();
        System.out.println(cengkeh);
        lineBreaker();
        System.out.println(kepalaSawit);
        lineBreaker();
        System.out.printf("%-25s: Rp. %,3.2f\n", "Uang Kas Saat Ini", cash);
    }
    ...
    public static void transactionHistory(){
        // Code here
        lineBreaker();
        if(transactions.isEmpty()) System.out.println("Belum ada daftar transaksi");
        else{
            for (Transaction transaksi : transactions) {
                lineBreaker();
    ...
    public static void nextDay(){
        // Code here
        day++;
        double totalProfit = ayam.dailyProfit() + sapi.dailyProfit() + domba.dailyProfit() + cengkeh.dailyProfit() + kepalaSawit.dailyProfit();
        cash += totalProfit;

        ayam.reduceDrugDuration();
        sapi.reduceDrugDuration();
        cengkeh.reduceDrugDuration();
        kepalaSawit.reduceDrugDuration();
        
        if(ayam.getMedicine() != null && !ayam.getMedicine().isInEffect()) ayam.setMedicine(null);
        if(sapi.getMedicine() != null && !sapi.getMedicine().isInEffect() ) sapi.setMedicine(null);
    ...
    public static void buyingConfirmation(String redaction, BuyableInterface item){
        ...
        Sc.nextLine();
        if(!isCashSufficient(countSpending(amount, item.getPricePerUnit()))) System.out.println("Maaf, Anda tidak memiliki uang kas yang cukup untuk melanjutkan transaksi");
        else{
            System.out.println("Apakah Anda yakin melanjutkan transaksi? (y/n)");
            char confirm = Sc.nextLine().toLowerCase().charAt(0);

            if(confirm == 'y'){
                cash -= item.getPricePerUnit() * amount;
                if(item instanceof Medicine) applyMedicine(((Medicine)item));
                else if(item instanceof BuyableInterface) item.addResource(amount);
                transactions.add(new Transaction(item.getName(), countSpending(amount, item.getPricePerUnit()), day, Helper.nameCapitalize(admin.getName())));
                System.out.println("Transaksi berhasil! Terima kasih telah berbelanja di sini");
            }
            else if(confirm == 'n') System.out.println("Anda telah membatalkan transaksi");
    ...
    public static void applyMedicine(Medicine medicine){
        // Code here
        if(medicine == boosterAyam) ayam.treatment(medicine);
        else if(medicine == boosterSapi) sapi.treatment(medicine);
        else if(medicine == boosterKepalaSawit) kepalaSawit.treatment(medicine);
        else if(medicine == pesticideCengkeh) cengkeh.treatment(medicine);
    }

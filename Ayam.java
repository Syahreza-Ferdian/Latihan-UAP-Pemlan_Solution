public class Ayam extends Hewan implements TreatmentInterface {
    ...
    public double dailySell(){
        // Code here
        return getEggPerDay() * this.getQuantity() * getEggPrice();
    }
    ...
    @Override public String toString(){
        // Code here
        return super.toString() + String.format("%-25s: %d butir (per ekor)\n%-25s: Rp. %,3.2f (per butir)\n%-25s: Rp. %,3.2f (per hari)\n%-25s: Rp. %,3.2f (per hari)\n%-25s: %s", 
            "Telur/hari", getEggPerDay(), "Harga Telur", getEggPrice(), "Penjualan", dailySell(), "Profit", dailyProfit(), "Status", getStatus());
    }
}
